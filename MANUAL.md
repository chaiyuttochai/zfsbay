...
# perccli64 — ถ้ายังไม่มี ดูวิธีติดตั้งใน 2.1.1
...
### 2.1.1 ติดตั้ง PERCCLI / StorCLI

มี 3 ทาง — เลือกตามสภาพเครื่อง:
...
# 2. ดาวน์โหลดจาก browser:
#    https://www.dell.com/support/home/en-us/product-support/servicetag/<TAG>/drivers
#    Filter: Category = SAS RAID, Operating System = Red Hat / SUSE Linux
#    หา "PERC Command Line Interface" → Download
#    ได้ไฟล์ PERCCLI_<version>_<rev>_Linux.tar.gz

# 3. โอนขึ้น Proxmox
scp PERCCLI_*_Linux.tar.gz root@<proxmox-host>:/tmp/

# 4. แตก + install (.deb อยู่ใน tar.gz)
cd /tmp
...
```bash
echo "deb http://hwraid.le-vert.net/debian bookworm main" \
  > /etc/apt/sources.list.d/hwraid.list
wget -qO- https://hwraid.le-vert.net/debian/hwraid.le-vert.net.gpg.key \
  | gpg --dearmor > /etc/apt/trusted.gpg.d/hwraid.gpg
apt update
apt install -y storcli

storcli64 show
```

⚠️ **ใช้ได้เฉพาะ Proxmox 8/bookworm** — bullseye repo ของ hwraid ไม่มี storcli (มีแค่ megacli รุ่นเก่าที่ syntax ต่างและ `zfsbay` ใช้ไม่ได้)

#### C. Broadcom StorCLI (rare — เครื่องไม่ใช่ Dell)

ดาวน์โหลด zip จาก https://www.broadcom.com/support/download-search → ค้น "StorCLI" → แตก → install `.deb`

#### หลังติดตั้งเสร็จ — ผูกกับ zfsbay

`zfsbay` ตรวจหา binary อัตโนมัติ แต่จะตั้ง explicit ก็ได้:

```bash
# ถ้าใช้ Dell PERCCLI
echo 'PERCCLI="/opt/MegaRAID/perccli/perccli64"' >> /etc/zfsbay.conf

# ถ้าใช้ Broadcom StorCLI
echo 'PERCCLI="/opt/MegaRAID/storcli/storcli64"' >> /etc/zfsbay.conf
